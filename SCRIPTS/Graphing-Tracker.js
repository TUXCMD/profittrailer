//Graphing-Tracker.js
//Script for ProfitTrailer v2 adding TradingView buttons, visual graph and PnL
//Add this script to the Custom JavaSCript addon for Chrome https://chrome.google.com/webstore/detail/custom-javascript-for-web/poakhlngfciodnhlhhgnaaelnpjljija
//Done
...
(function(){
    ...
    util.graphMinutes = 5; //how many minutes of graph to show?
...
function estimate() {
    ...
    if ($('#dtPairsLogs thead').length > 0) {
        ...
        $('#dtPairsLogs tbody tr').each(function() {
           var num3 = $(this).find('td:nth-child(9)').text();
           var num3txt = $(this).find('td:nth-child(9)').html();
           var allTriggers = $(this).find('td.text-right.trigger').html();
           var SplitTriggers = $(this).find('td.text-right.trigger').html().split('<br>')[0];
           var TriggerPercentage = parseFloat(SplitTriggers).toFixed(2);
           var TriggerToPercentage = (parseFloat(100) + parseFloat(TriggerPercentage)) / 100;
           //console.log("TriggerPercentage % = " + TriggerPercentage);
           //console.log("TriggerToPercentage % = " + TriggerToPercentage);
           var calc2 = num3 * TriggerToPercentage;
           var calc3 = calc2.toFixed(8);
           if ($(this).find('.current-price').hasClass("selltarget")) {
            $(this).find('.selltarget').html('<br>Target: ' + calc3);
            $()
           }
           else
           {
            $(this).find('.current-price').append('<span class="selltarget" style="color:#05b16f;"><i style="color:#98a6ad;font-style:normal;"><br>Target: ' + calc3 + '</span>');
            $(this).find(".selltarget:eq(1)").remove()
           }
          });      
    ...
    if ($('#dtDcaLogs thead').length > 0) {
        ...
        //DCA TARGET  
         $('#dtDcaLogs tbody tr').each(function() {
           var num3 = $(this).find('td.text-right.blue-color.avg-price.current-price > span').html().split('<br>')[1].split("<span")[0];
           console.log ("Avg. pr = "+num3);
           var num3txt = $(this).find('td:nth-child(9)').html();
           //var allTriggers = $(this).find('td.text-right.trigger').html();
           var SplitTriggers = $(this).find('td:nth-child(11)').html().split('<br>')[0];
           var TriggerPercentage = parseFloat(SplitTriggers).toFixed(2);
           var TriggerToPercentage = (parseFloat(100) + parseFloat(TriggerPercentage)) / 100;
           //console.log("TriggerPercentage % = " + TriggerPercentage);
           //console.log("TriggerToPercentage % = " + TriggerToPercentage);
           var calc2 = num3 * TriggerToPercentage;
           var calc3 = calc2.toFixed(8);
           if ($(this).find('.dca-avg-current-price').hasClass("selltarget")) {
            $(this).find('.selltarget').html('<br>Target: ' + calc3);
            $()
           }
           else
           {
            $(this).find('.dca-avg-current-price').append('<span class="selltarget" style="color:#05b16f;text-align:left !important"><i style="color:#98a6ad;font-style:normal;"><br>Target: ' + calc3 + '</span>');
            $(this).find(".selltarget:eq(1)").remove()
           }
          });      
